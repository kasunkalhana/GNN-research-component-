...
class AttentionAggregator(nn.Module):
    ...
    def forward(self, src: torch.Tensor, dst: torch.Tensor, edge_index: torch.Tensor) -> torch.Tensor:
        if edge_index.numel() == 0:
            return torch.zeros(
                (dst.size(0), self.src_lin.out_features),
                device=dst.device,
                dtype=dst.dtype,
            )
...
class RecurrentHGAT(nn.Module):
    ...
    def forward(
        ...
        lane_x = data["lane"].x if "lane" in data else None
        junction_emb = self.encoders["junction"](junction_x)
        lane_emb = self.encoders["lane"](lane_x) if lane_x is not None else None

        lane_to_junction_edges = data[("lane", "to", "junction")].edge_index
        if lane_emb is None:
            lane_messages = torch.zeros_like(junction_emb)
        else:
